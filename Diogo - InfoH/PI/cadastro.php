...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Se o tipo de usuário for administrador, verifica se já existe um administrador
    if ($tipo_usuario == 'administrador') {
        // Verifica se já existe um administrador no banco de dados
        $stmt = $db->prepare("SELECT id FROM usuarios WHERE tipo_usuario = 'administrador'");
        $stmt->execute();
        $stmt->store_result();

        // Se já existir um administrador, bloqueia a criação de um novo
        if ($stmt->num_rows > 0) {
            echo "<script>alert('Já existe um usuário administrador.'); history.back();</script>";
            exit;
        }
    }
