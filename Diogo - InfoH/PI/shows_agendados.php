...
// Preencher o array de espaços corretamente
$stmt = $db->prepare("SELECT id, nome_espaco, cidade FROM espacos");
$stmt->execute();
$result = $stmt->get_result();
            ...
            <tbody>
                <?php foreach ($solicitacoes as $solicitacao): ?>
                    <tr>
                        ...
                        <td>
                            <?php 
                            if (isset($espacos[$solicitacao['espaco_id']]) && is_array($espacos[$solicitacao['espaco_id']])) {
                                echo htmlspecialchars($espacos[$solicitacao['espaco_id']]['nome_espaco']);
                            } else {
                        ...
                        <td>
                            <?php 
                            if (isset($espacos[$solicitacao['espaco_id']]) && is_array($espacos[$solicitacao['espaco_id']])) {
                                echo htmlspecialchars($espacos[$solicitacao['espaco_id']]['cidade']);
                            } else {
